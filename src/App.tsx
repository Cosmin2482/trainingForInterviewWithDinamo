...
import { Menu, BookOpen, LayoutGrid, Swords } from 'lucide-react';

export default function App() {
  const [activeModuleIndex, setActiveModuleIndex] = useState(0);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [viewMode, setViewMode] = useState<'learn' | 'glossary' | 'quiz'>('learn');
  ...
  return (
    <div className="flex h-dvh bg-neutral-50 font-sans text-neutral-900 overflow-x-hidden">
      {/* Sidebar */}
      <Sidebar 
      ...
      {/* Main Content */}
      <main className="flex-1 flex flex-col h-dvh relative overflow-hidden lg:ml-0">
        {/* Header */}
        <header className="p-4 bg-white border-b border-neutral-200 flex items-center justify-between shadow-sm z-30 relative">
          <div className="flex items-center gap-4">
            ...
            <span className="font-black text-neutral-900 truncate max-w-[200px] lg:hidden uppercase italic">
              {viewMode === 'glossary' ? 'Glosar' : viewMode === 'quiz' ? 'Quiz Arena' : activeModule.title}
            </span>
          </div>
          ...
          <div className="flex bg-neutral-100 p-1 rounded-lg border border-neutral-200">
            ...
            >
              <Swords size={16} />
              <span className="hidden sm:inline">Meci</span>
            </button>
          </div>
        </header>

        {/* Content Area */}
        <div className="flex-1 min-h-0 bg-neutral-50 relative overflow-hidden">
          ...
          <div className="relative z-10 h-full min-h-0 overflow-hidden">
            {viewMode === 'glossary' ? (
              ...
              <div className="h-full w-full min-h-0 overflow-y-auto overflow-x-hidden">
                <ConceptCard 
